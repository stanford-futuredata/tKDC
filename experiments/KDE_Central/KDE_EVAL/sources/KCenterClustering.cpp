	...
	)
{
	...
	for(int k=0; k<K; k++)
	{
		//printf("pNumPoints [%d] = %d\n", k, pNumPoints[k]);		
		for(int dim=0; dim<d; dim++)
		{
			pClusterCenters[(k*d)+dim]=pClusterCenters[(k*d)+dim]/pNumPoints[k];
		}
		//printf("pClusterCenters[(k*d)+dim] = (%f,%f), pNumPoints [%d] = %d\n", pClusterCenters[(k*d)+0],pClusterCenters[(k*d)+1], k, pNumPoints[k]);
	}
